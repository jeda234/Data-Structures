...
template <class T> UnrolledLL<T>& UnrolledLL<T>::operator=(UnrolledLL<T>& old) {
...
template <class T> void UnrolledLL<T>::push_front(const T& v) {
  if (!tail_) { // special case: initially empty list
    ...
    if (head_->num_elements >= num_elements_per_node) { // checking to see if you are out of bounds
      Node<T>* newp = new Node<T>(v);
      newp->elements[0]=v; // next few lines link the nodes together
      newp->next_ = head_;
      head_->prev_ = newp;
      head_ = newp;
      head_->elements[0]=v;
...
template <class T> void UnrolledLL<T>::pop_front() {
  if (size_!=0) {
    if ((head_->num_elements)==1) {// deleting the node
      Node<T>* newp=head_;
      head_=head_->next_;
      delete newp;
      newp=NULL;
    } else { // pushing the elements to the front of the node
...
template <class T> void UnrolledLL<T>::push_back(const T& v) {
  if (tail_==NULL) { // special case: initially empty list
    ...
    if (tail_->num_elements >= num_elements_per_node) { // checking to see if you are out of bounds
      Node<T>* newp = new Node<T>(v);
      newp->elements[0]=v; // linking the nodes together
      newp->prev_ = tail_;
      tail_->next_ = newp;
      tail_ = newp; 
      ++(newp->num_elements);
...
template <class T> void UnrolledLL<T>::pop_back() {
  if (size_!=0) { // making sure the list has contents
    if ((tail_->num_elements)==1) { // deleting the node
      tail_=tail_->prev_;
      delete tail_->next_;
      tail_->next_=NULL;
      ...
      tail_->elements[(tail_->num_elements)-1]=0; // making it equal 0
      --(tail_->num_elements); // the pointer location
    } 
  }
  size_--;
}
...
template <class T> typename UnrolledLL<T>::iterator UnrolledLL<T>::insert(iterator itr, const T& v) {
  ++size_;
  if (itr.ptr_->num_elements < num_elements_per_node) { // insert into node
    if (itr.offsets==0) {
    ...
    int x = itr.offsets; // in order to perserve the place where it should be inserted
    itr.offsets++; // putting it here makes it run better
    itr.ptr_->elements[x]=v;
    ++(itr.ptr_->num_elements);
    itr.ptr_->num_elements-itr.offsets;
  } else { // creating a new node a pushing other elements after the insert into a new node
    Node<T>* newp1 = new Node<T>();
    for (int i=0; i<itr.offsets; i++) {
      newp1->elements[i]=itr.ptr_->elements[i];
    }
    newp1->elements[itr.ptr_->num_elements-itr.offsets]=v;
    newp1->num_elements=itr.offsets+=1;
    int x = itr.ptr_->num_elements;
    for (int i=0; i<x-itr.offsets; i++) {
      for (int j=itr.offsets; i<itr.ptr_->num_elements; i++) {
        itr.ptr_->elements[i]=itr.ptr_->elements[j];
        --(itr.ptr_->num_elements);
      }
    }
    newp1->prev_=itr.ptr_->prev_;
    newp1->next_=itr.ptr_;
    if (itr.ptr_ == head_) {
      head_=newp1;
    } else {
      newp1->prev_->next_=newp1;
    }
  }
  return itr;
}
...
template <class T> void UnrolledLL<T>::copy_list(UnrolledLL<T>& old) { 
  ...
  for (UnrolledLL<T>::iterator old_p = old.begin(); old_p != old.end(); old_p++) {  // copy the list
    this->push_back(*old_p); // push_back should handle creation of new nodes.
  }
  size_ = old.size_;
}

template <class T> void UnrolledLL<T>::destroy_list() {
  Node<T> *current = head_; // emptying and deleting everything
  Node<T> *next;
  while (current != NULL) {
    next = current->next_;
    delete current;
